...
Granite includes a simple, secure authentication system for single-user deployments. When enabled, users must log in with a password before accessing the application.
...
# Or with docker run
docker exec -it granite python generate_password.py
```
...
**Docker Users:**
```bash
docker-compose exec granite python -c "import secrets; print(secrets.token_hex(32))"

# Or with docker run
docker exec -it granite python -c "import secrets; print(secrets.token_hex(32))"
```
...
5. **VPN/Private Network** - Keep Granite on a private network or behind a VPN
