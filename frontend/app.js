...
function noteApp() {
    return {
        ...
        updateEditorContent(content) {
            ...
            // Use transaction instead of setState to avoid DOM sync issues
        ...
        // CodeMirror handles syntax highlighting natively - old functions removed
        ...
        onNoteDragStart(notePath, event) {
            ...
            // Set draggedNote for both notes and images (used by folder drop logic)
        ...
        async onEditorDrop(event, view) {
            ...
            // Insert at cursor position (CodeMirror will handle this via view parameter)
            if (view) {
                ...
                // Fallback if not called from CodeMirror
        ...
        async onFolderDrop(targetFolderPath) {
            ...
            // Handle note/image drop into folder
            if (this.draggedNote) {
                const item = this.notes.find(n => n.path === this.draggedNote);
                if (!item) return;

                // Get filename (works for both notes and images)
                ...
                try {
                    ...
                    if (response.ok) {
                        await this.loadNotes();
                        // Keep current item open if it was the moved item
        ...
        async loadNote(notePath, updateHistory = true, searchQuery = '') {
            try {
                ...
                this.updateEditorContent(data.content); // Update CodeMirror editor
        ...
        insertLink() {
            ...
            const urlStart = selection.from + linkText.length + 3; // After "[linkText]("
...
`;
            ...
            this.editorView.dispatch({
                ...
                selection: { anchor: newPos, head: newPos + 8 } // Select "Header 1"
            });
        ...
        get renderedMarkdown() {
            ...
            // Convert Obsidian-style image embeds: ![[image.png]] or ![[image.png|alt text]]
            // Must be done before regular wikilink processing
            const allImages = this.notes.filter(n => n.type === 'image'); // Get all images
            contentToRender = contentToRender.replace(
                ...
                (match, target, altText) => {
                    ...
                    // Find image by filename (search anywhere in notes directory)
            ...
            images.forEach(img => {
                ...
                if (src) {
                    ...
                    if (!src.startsWith('http://') && !src.startsWith('https://') &&
                        ...
                        // If it's a relative path (doesn't start with /), resolve it relative to current note's folder
                        if (!src.startsWith('/')) {
                            ...
                            // Prepend the folder path if it exists
                            if (currentNoteFolder) {
                                imagePath = `${currentNoteFolder}/${src}`;
                            }
                        } else {
                            // Remove leading slash for absolute paths within notes directory
        ...
        setupScrollSync() {
            // Get CodeMirror scroller and preview container
            const editorScroller = this.editorView?.scrollDOM;
            const preview = this._domCache.previewContainer || document.querySelector('.preview-container');
        ...
        async togglePlugin(pluginId, enabled) {
            try {
                ...
                // Reload all plugins to update UI and plugin states
    ...
    <div class="markdown-preview">
        ...
        async exportToPDF() {
            ...
            try {
                ...
                // Make API call to PDF export endpoint
