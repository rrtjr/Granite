"""
Note Statistics Plugin for Granite
...
class Plugin:
    ...
    def format_stats(self, stats: dict) -> str:
        ...
        if stats["links"] > 0:
            lines.append(
                f"  Links: {stats['links']} ({stats['internal_links']} internal, {stats['external_links']} external)"
            )
        ...
        if stats["tasks"]["total"] > 0:
            lines.append(
                f"  Tasks: {stats['tasks']['completed']}/{stats['tasks']['total']} completed ({stats['tasks']['completion_rate']}%)"
            )

        if stats["headings"]["total"] > 0:
            lines.append(
                f"  Headings: {stats['headings']['total']} (H1: {stats['headings']['h1']}, H2: {stats['headings']['h2']}, H3: {stats['headings']['h3']})"
            )
